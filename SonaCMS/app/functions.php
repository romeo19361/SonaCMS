...
//
// SELF-GUARDING: the whole file is wrapped so it can be included more than once
// per request (e.g. by a page directly and transitively via another include)
// without a "cannot redeclare" fatal. Same robustness pattern as paths.php.
if (!defined('SONA_FUNCTIONS_LOADED')) {
    define('SONA_FUNCTIONS_LOADED', true);
    ...
    /**
     * Apply the site's timezone from config so all date()/time formatting across
     * the app (activity log, publish dates, etc.) uses the site's local time
     * rather than the server's default. SonaCMS ships worldwide, so the timezone
     * is a per-install config value, not hardcoded.
     *
     * Falls back to 'UTC' if not set or invalid — safe and unambiguous, so older
     * installs (or a config that omits it) still get sane, consistent times.
     *
     * Call once, early, wherever config is loaded (frontend and admin entry points).
     *
     * @param array $config The config array (from config.php).
     */
    function applyTimezone(array $config): void
    {
        $tz = $config['timezone'] ?? 'UTC';
        // Validate against PHP's known zones; fall back to UTC if unrecognised.
...
} // end SONA_FUNCTIONS_LOADED guard